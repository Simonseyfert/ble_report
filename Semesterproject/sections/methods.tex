...
via a CPU-polling loop --- the maximum rate this approach could sustain, with no path to
multi-channel acquisition. This approach had a fundamental limitation: the Nordic SoftDevice
...
causing missed samples --- likely even at its reported 2\,kHz single-channel maximum. The reported 90\,kbps data rate furthermore reflects synthetic
...
its additional RAM, USB, nor its extended radio features, such as Bluetooth Long Range,
Direction Finding, Thread, or Zigbee, are required), the
...
instances, and BLE 5.3. No feature of the nRF52840 is required by the present design.
...
response --- both clocked by SCLK. A chip-select (CS) line, set to low by the master
...
peripheral to send the same command on every transaction, with the
...
\texttt{CONVERT(0..15)} sequence correctly --- an additional bookkeeping step, though
not fundamentally difficult, that is absent from the present design. Only samples from the channels selected by the user in the GUI
(internally represented as a 16-bit bitmask) are forwarded to the receive buffer
...
not achievable in practice. This mode required only a small firmware change ---
substituting the \texttt{CONVERT($k$)} command word for \texttt{CONVERT(63)} --- and no
channel bitmask filtering, since every received sample already corresponds to the
single selected channel. The
...
use, \texttt{VSTIM+} can be connected to \texttt{VDD} and \texttt{VSTIM-} to ground. When
...
the chip's headstage was connected via an Omnetics connector to a larger carrier
board (Figure~\ref{fig:intan_board}).
...
\begin{figure}[h]
    ...
    \caption{Intan RHS2116 chip (black die) on its small headstage, connected via an
    Omnetics connector to a larger carrier board, which provides accessible pin
    headers for bench-top SPI connections during development.}
...
proactively sends data to the central at each connection interval.
...
implementation effort. In doing so it occupies a reserved
...
external J-Link connection as described in Appendix~\ref{app:flash_ndk}. Software requirements for running the system and
...
operation and firmware flashing, plus two LEDs retained for debugging visibility,
with no buttons or other peripherals. The final
...
\begin{figure}[h]
    ...
    and the standard Intan headstage connector. The QFN48 module is shown.}
...
A real test signal could not be used for this measurement: as shown above, the
function generator's own cables pick up interference inside the WPT field, which
would confound the measurement. An on-board oscillator providing a clean reference
signal without external cables (as in the flex PCB prototype, Section~\ref{sec:results_noise})
is the intended long-term solution, but was not available for this test. Instead,
...
start of each acquisition to produce the required inter-transaction interval, so that
a new sample is acquired according to the selected sampling rate. The
...
Combining this with the 16\,MHz clock rate (16\,ticks/\textmu s) gives a simple
expression for the timer period:
\begin{equation}
...
giving a total SPI rate of 32\,kHz --- meaning a new sample, from one of the 16
channels, arrives every 31.25\,\textmu s. In single-channel mode, the factor of 16 does
...
\texttt{DMA\_BUF\_SAMPLES}), the \texttt{timer2\_count\_handler} interrupt service
routine (ISR) fires. The
...
requires no CPU and continues regardless, so additional data samples accumulate
...
Because the timer2 interrupt that triggers the swap can be delayed (as described
above), the swap can land on any channel within the 16-channel cycle, not just at
the start of a new one --- the buffer boundary is not guaranteed to align with
channel~1. Even without any delay, the nominal swap threshold (819 slots) is not a
multiple of 16 anyway: $819 \bmod 16 = 3$, so the next DMA slot would correspond to
channel~4 (the firmware's 0-based indexing gives channel index 3). The variable
...
consecutive missed BLE connection events (at the 10\,ms interval) before overflow,
provided the connection subsequently catches up. Some catch-up margin is expected
to exist beyond the steady-state 32\,kHz rate: instantaneous throughput fluctuates
slightly during normal operation without packet loss, and earlier development
versions of the firmware operated reliably at per-channel rates above 2\,kHz, both
indicating headroom in the BLE link beyond what is strictly required at the default
recording rate.
...
(reader) cannot run simultaneously: each side modifies only its own pointer, so
neither can read a value while the other is in the middle of changing it, which
...
time this switch happens. In its base state, chip-select (CS) is controlled directly
...
and the PPI links are enabled, the full acquisition loop --- pulling CS low,
performing the SPI transaction, releasing CS high again, and writing the result to
the active buffer via DMA --- runs entirely in hardware with no per-sample CPU
involvement.
...
recording-rate value --- a conservative choice within the stable operating range;
160\,kHz was tested and found to cause unreliable operation. EasyDMA is
reconfigured to transmit the pre-built
...
using PPI and EasyDMA. The TX buffer holds the full single-pulse waveform --- the
cathodic phase, interphase delay, and anodic phase --- as a pre-built sequence of
SPI command words; EasyDMA simply transmits this sequence once per pulse. Because
EasyDMA lists are not ring buffers, they do not automatically wrap back to the
start once exhausted, so the CPU must reset the TX pointer before each new pulse
can be sent. This reset naturally fits inside the inter-pulse gap that already
separates consecutive pulses (the idle time between the anodic phase of one pulse
and the cathodic phase of the next), requiring no additional timing mechanism. The
only CPU involvement during active stimulation is therefore a single ISR, triggered
during this gap, that resets the pointer so the next pulse replays the same
waveform from the start (Section~\ref{sec:stim_ppi}). For
each channel, the stored parameters are an enable flag, the cathodic and anodic
DAC values, and the cathodic and anodic pulse durations expressed as transaction
counts.
...
mechanism so that the change takes effect on all channels together, with no delay
between them. These two transactions consume
16\,\textmu{}s of the phase period but are necessary to initiate the current.
Adding $0.5$ before taking the floor rounds to the nearest integer rather than
truncating downward. All pulse durations are therefore
...
channel; per-channel timing is not supported in the current implementation. This
would be a non-trivial extension, since the entire pulse sequence is driven by a
...
\begin{enumerate}
    ...
    polarity and enable the current source on all selected channels, using the
    Intan's triggered-update (U-flag) mechanism so that both changes apply
    together with no delay between them. A flushing transaction follows, then
    $(\texttt{cat\_n} - 2)$ further \emph{dummy} transactions: these write nothing
    new, but each one occupies an 8\,\textmu{}s slot during which the current
    source already enabled above keeps running, simply holding the cathodic
    current on for the remainder of the phase.

    \item \textbf{Phase transition}: the same triggered-update mechanism allows
    two different transition behaviours. If the interphase delay is zero, the new
    polarity and stimulator state are pre-loaded (without yet taking effect)
    during the final two cathodic dummy slots, then applied together in a single
    transaction: the polarity flips to anodic and stimulation stays continuously
    on, with no gap in current between phases. If the interphase delay is
    non-zero, the current source is instead switched off at the end of the
    cathodic phase, dummy transactions time the gap, and the current source is
    switched back on with anodic polarity in the final transaction of the gap.
    Like pulse widths, the interphase delay is quantised to multiples of
    8\,\textmu{}s; at the minimum non-zero delay (one transaction, 8\,\textmu{}s),
    that single transaction is the anodic-enable command itself, with no separate
    dummy transaction in between.

    \item \textbf{Anodic phase}: \texttt{ano\_n} dummy transactions (as defined
    above) sustain the anodic current.
...
Building the full sequence above also gives the total number of transactions in
one pulse; this total is computed once, before stimulation starts, and configures
Timer4's compare value so that Timer4 can later detect when a complete pulse has
been sent. The same total also determines how long the gap before the next pulse
must be, as described next.
...
additional PPI links for pulse sequencing. Recording uses a PPI link that counts
each completed SPI transaction toward the DMA buffer-swap threshold; this link is
the only one from recording mode that is disabled during stimulation, since there
is no DMA receive buffer to swap. The three links that drive the SPI transaction
itself (timer tick, CS low, CS high) continue unchanged, and three new links
(described below) handle pulse-level sequencing instead. The complete set of six
active PPI links during stimulation is summarised in Table~\ref{tab:ppi_stim}.
...
The gap timer's duration (and, via the table above, when it restarts Timer1 for the
next pulse) is set so that the inter-pulse gap is:
\begin{equation}
...
(PPK2)~\cite{ppk2} in current-measurement mode at 10\,kSPS, supplied by a regulated
DC bench supply at a defined constant voltage. Average and peak current values are read from
...
headroom for resistive losses in thin-film interconnects). Table~\ref{tab:power_configs} lists all operating configurations measured.
...
as formatted decimal strings (each 16-bit sample can be up to 5 digits, plus a
separator character, giving 6 bytes per sample in text form). The packet size of
112 samples dates back to an earlier design stage when only the full 16-channel
configuration was planned: 112 is the largest integer divisible by 16 whose
payload (112\,$\times$\,2\,$+$\,2 bytes counter\,$=$\,226 bytes) fits within the
maximum packet length (247 bytes). Divisibility by 16 meant that, in that
16-channel-only configuration, each packet contained exactly seven complete
channel cycles and therefore always began at channel~1, simplifying channel
demultiplexing. This property does not carry over to the variable channel-count
...
\begin{itemize}
    ...
    detect the beginning of each packet in the raw RTT byte stream;
    \item Bytes 6--229: $112 \times 2$-byte sample values in big-endian order
    (most significant byte first);
    \item Bytes 230--231: packet counter, a 2-byte value in big-endian order.
\end{itemize}
...
commands arriving while disconnected are silently discarded. Two categories of
recognised commands are forwarded to the peripheral:
...
The RTT stream is a raw byte stream with no built-in framing. The background
reader thread maintains an internal byte buffer and searches for the 6-byte synchronisation
...
misalignment. Once a complete 232-byte frame is confirmed (packet loss detection
is described below), the 112 interleaved sample values are sorted into
per-channel buffers by following a rolling index that cycles through the active
channels in order; this index carries over from one packet to the next, so
...
these settings are packed into a binary R~packet (7\,bytes) and sent to the
peripheral. Data are saved to a timestamped CSV file in batches of 1000
...
Channel desynchronisation refers to all recorded channels appearing
consistently shifted in the GUI, as if every sample had been assigned to the
wrong channel by the same fixed offset. Under normal operating conditions with a
stable power supply, no channel desynchronisation was observed during continuous
operation. Occasional
desynchronisation was observed in some conditions, including but not limited to
operation when the supply voltage was close to the lower edge of the acceptable
range. Insufficient supply voltage is one plausible cause, though the exact
mechanism by which it would lead to desynchronisation was not identified --- it
could affect any part of the system, not necessarily SPI signalling specifically.
Another, more concretely identified mechanism is noise on the CS line: a spurious
CS pulse causes the Intan RHS2116 to advance its internal channel counter by one
extra step, permanently offsetting all subsequent channel assignments by a
constant amount (identical across all channels). Line noise can corrupt data on
the SPI bus directly, and insufficient power can cause other unrelated problems
elsewhere in the system; in both cases, channel desynchronisation should be
treated as a symptom of a more fundamental operating problem --- not a firmware
defect --- and investigated accordingly. Noise on the CS line, or on SPI signals
more generally, originating from the long inter-board wiring is expected to be
reduced when the Intan and nRF52832 are collocated on the same PCB with short SPI
traces.
...
artefacts on the project repository\footnote{\url{https://github.com/Simonseyfert/ble_spi_intan}
(private repository; access available upon request at
\href{mailto:simon.seyfert@epfl.ch}{simon.seyfert@epfl.ch} or
\href{mailto:simon.seyfert@tum.de}{simon.seyfert@tum.de}).}
(only the Windows build was verified during development),
