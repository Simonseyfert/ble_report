...
(3.0$\times$3.2\,mm, 49 solder balls~\cite[p.~545]{nordic_nrf52832_ps})
...
GND/REF). With the green carrier board and the function generator cables far from
...
nRF52832 via SPIM0), drives the clock and initiates every transaction, while the
other, the slave (here the Intan RHS2116), responds only when addressed. The
chip-select (CS) line is an active-low signal that the master holds low for the
duration of each transaction, signalling to the slave that it should respond;
returning it high ends the transaction. GPIOTE (General-Purpose I/O Tasks
...
only samples whose channel index has its corresponding bit set in
\texttt{g\_channel\_mask} --- a 16-bit bitmask in which bit~$n$ enables
channel~$n$, configured by the GUI at recording start --- are forwarded; the
running variable \texttt{g\_channel\_phase} tracks which physical channel each
DMA slot corresponds to across buffer boundaries, since the \texttt{CONVERT(63)}
cycle does not reset at buffer swap boundaries. In single-channel mode, all
samples are copied directly. The ring buffer holds \texttt{RING\_BUF\_SAMPLES}
...
$(\texttt{last\_counter} + 1) \bmod 65536$; if the received value differs, the
estimated number of lost packets is
$\texttt{lost} = (\texttt{counter\_raw} - \texttt{expected}) \bmod 65536$.
A threshold of $\texttt{lost} < 65534$ excludes the degenerate cases where the
counter appears to have moved backward by one or two steps, which cannot occur
under normal monotonic operation and would indicate a spurious condition. When
genuine loss is detected, the cumulative loss counter is incremented and the
channel interleaving phase is corrected: \texttt{ch\_phase} is advanced by
$112 \times \texttt{lost}$ modulo the active channel count, maintaining correct
